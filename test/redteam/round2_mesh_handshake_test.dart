...
void main() {
  test('unverifiable address must not become a reachable peer', () async {
    ...
    expect(ok, isFalse,
        reason: 'connectToPeer returned true for an unreachable, '
    ...
    expect(peers, isEmpty,
        reason: 'phantom peer was marked isReachable and now appears in '
  ...
  test('phantom reachability must not unlock payload routing', () async {
    ...
    expect(tier, isNull,
        reason: 'a peer that never exchanged a byte got a transport route; '
  ...
  test('bootstrap candidates must stay unproven until a real handshake',
    ...
    expect(ok, isFalse,
        reason: 'bootstrap handshake "succeeded" against a peer no test '
