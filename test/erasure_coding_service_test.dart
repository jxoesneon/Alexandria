  ...
  group('ErasureCodingService Tests', () {
    ...
    test('encodes and decodes data with all shards available', () {
      final original = Uint8List.fromList(
          'Hello Cauchy Reed-Solomon Erasure Coding!'.codeUnits);
      final block =
          service.encode(blockId: 'blk_1', data: original, k: 4, m: 2);
      ...
      final decoded =
          service.decode(block: block, availableShards: block.shards);
      expect(decoded, equals(original));
    });

    test('decodes data when 2 parity shards are lost', () {
      final original = Uint8List.fromList(
          'Data preservation under high churn mesh!'.codeUnits);
      final block =
          service.encode(blockId: 'blk_2', data: original, k: 4, m: 2);
    ...
    test('decodes data when 2 data shards are lost (recovering from parity)',
        () {
      final original = Uint8List.fromList(
          'Recovering from parity shards over Galois Field!'.codeUnits);
      final block =
          service.encode(blockId: 'blk_3', data: original, k: 4, m: 2);
    ...
    test('throws StateError if available valid shards < k', () {
      final original =
          Uint8List.fromList('Insufficient shards test payload'.codeUnits);
      final block =
          service.encode(blockId: 'blk_4', data: original, k: 4, m: 2);

      final available =
          block.shards.sublist(0, 3); // Only 3 shards when 4 are required
      expect(() => service.decode(block: block, availableShards: available),
          throwsStateError);
    });

    test('repairs missing shards cleanly', () {
      final original =
          Uint8List.fromList('Peer-to-Peer Resilient Mesh'.codeUnits);
      final block =
          service.encode(blockId: 'blk_5', data: original, k: 4, m: 2);
      ...
      final repairedBlock =
          service.repairShards(block: block, availableShards: partialShards);
