...
void main() {
  group('threshold verification', () {
    test(
        'a valid 3-of-5 signed manifest is accepted and raises the '
        'floor', () async {
    ...
    test('a forged release signature never counts', () async {
      ...
      expect(
          await auth
              .ingest(SignedManifestBundle(manifest: forged, timestamp: ts)),
          ManifestIngestResult.insufficientReleaseSignatures);
    });
    ...
    test('duplicate keyIds count once — a signature flood is one vote',
      ...
      expect(
          await auth
              .ingest(SignedManifestBundle(manifest: flooded, timestamp: ts)),
          ManifestIngestResult.insufficientReleaseSignatures);
    });

    test(
        'wrong-role signatures never count — release slot signed by '
      ...
      final badTsPreimage = ManifestTimestamp.signingPreimage(
          ...
          expiresAt: _t0 + 600000);
      final relTsSig =
          await Ed25519().sign(badTsPreimage, keyPair: c.release['rel-1']!);
      ...
      expect(
          await auth
              .ingest(SignedManifestBundle(manifest: m2, timestamp: badTs)),
  ...
  group('floor semantics (raise-only, never lower)', () {
    ...
    test(
        'a floor below a previously accepted manifest refuses — the '
    ...
    test(
        'a floor equal to baseline is accepted (ratchets sequence '
  ...
  group('staleness / rollback / binding', () {
    test('expired manifest refuses', () async {
      ...
      final bundle = await _bundle(
          ...
          tsExp: _t0 + 600000);
      expect(await auth.ingest(bundle), ManifestIngestResult.expiredManifest);
    });

    test('stale timestamp record refuses (freeze/replay protection)', () async {
      final c = await _SignerQuorum.create();
      final auth = _authority(await c.registry());
      final bundle = await _bundle(
          relKeys: _take(c.release, ['rel-1', 'rel-2', 'rel-3']),
      ...
      expect(await auth.ingest(bundle), ManifestIngestResult.staleTimestamp);
    });

    test(
        'a timestamp bound to a different manifest hash or sequence '
        'is malformed', () async {
      final c = await _SignerQuorum.create();
      final auth = _authority(await c.registry());
      final m = await ReleaseManifest.issue(
      ...
      expect(
          await auth
              .ingest(SignedManifestBundle(manifest: m, timestamp: tsWrong)),
      ...
      expect(
          await auth
              .ingest(SignedManifestBundle(manifest: m, timestamp: tsHash)),
          ManifestIngestResult.malformed);
    });
    ...
    test('rollback: a non-increasing sequence refuses after acceptance',
      ...
      expect(
          await auth.ingest(
              await _bundle(relKeys: keys, tsKeys: c.timestamp, seq: 5)),
      ...
      expect(
          await auth.ingest(
              await _bundle(relKeys: keys, tsKeys: c.timestamp, seq: 4)),
          ManifestIngestResult.rollback);
      // A strictly-greater sequence still passes.
      expect(
          await auth.ingest(
              await _bundle(relKeys: keys, tsKeys: c.timestamp, seq: 6)),
          ManifestIngestResult.accepted);
      expect(auth.acceptedSequence, 6);
    });

    test('the rollback cursor honours the persisted-sequence seam', () async {
      final c = await _SignerQuorum.create();
      var persisted = 0;
      final auth = _authority(await c.registry(),
      ...
      expect(
          await auth.ingest(
              await _bundle(relKeys: keys, tsKeys: c.timestamp, seq: 3)),
          ManifestIngestResult.rollback);
      expect(
          await auth.ingest(
              await _bundle(relKeys: keys, tsKeys: c.timestamp, seq: 7)),
          ManifestIngestResult.accepted);
  ...
  group('registry + envelope transport', () {
    test('the empty registry fails closed — nothing is ever trusted', () async {
      final c = await _SignerQuorum.create();
    ...
    test(
        'a manifest envelope signed by a release key is ingested; a '
      ...
      expect(await auth.ingestEnvelope(badEnv), ManifestIngestResult.malformed);
      ...
      expect(
          await auth.ingestEnvelope(wrongKind), ManifestIngestResult.malformed);
    });
    ...
    test('manifest JSON round-trips through the payload block', () async {
      ...
      final parsed = SignedManifestBundle.fromPayload(bundle.toPayloadBlock());
