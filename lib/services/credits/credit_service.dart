...
class CreditService extends ChangeNotifier {
  ...
  double awardStorageCredits({
    ...
    // Dynamic Rarity Weight Function (ALX-005 §4.1)
    final double rarityWeight;
    if (peerCount <= 1) {
      rarityWeight = 5.0; // Critically Endangered
    } else if (peerCount == 2) {
      rarityWeight = 3.0; // Vulnerable
    } else if (peerCount < 5) {
      rarityWeight = 1.5; // Near-Safe
    } else {
      rarityWeight = 1.0; // Healthy
    }

    final mbSize = sizeBytes / (1024 * 1024);
    final earned = (mbSize * 0.1 * rarityWeight).clamp(0.1, 50.0);
  ...
  double awardComputeCredits({
    ...
    final earned = (2.0 * cauchyMb) + (0.5 * fastCdcMb) + (5.0 * ocrPages);
  ...
  ImpressionReceipt awardSponsorshipKickback({
    ...
    final clientKickback = grossCredits * 0.85;
