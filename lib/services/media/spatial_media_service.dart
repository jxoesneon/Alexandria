...
class SpatialMediaService {
  GlbMetadata parseGlbHeader(Uint8List bytes) {
    if (bytes.length < 20) throw FormatException('Truncated GLB buffer');
    // "glTF" magic 0x46546C67
    if (bytes[0] != 0x67 || bytes[1] != 0x6C || bytes[2] != 0x54 || bytes[3] != 0x46) {
      throw FormatException('Invalid glTF GLB binary signature');
    }

    final version = bytes[4] | (bytes[5] << 8) | (bytes[6] << 16) | (bytes[7] << 24);
    final length = bytes[8] | (bytes[9] << 8) | (bytes[10] << 16) | (bytes[11] << 24);
    final jsonChunkLen = bytes[12] | (bytes[13] << 8) | (bytes[14] << 16) | (bytes[15] << 24);
