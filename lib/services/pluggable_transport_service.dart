...
final pluggableTransportServiceProvider = Provider((ref) => PluggableTransportService());
...
class PluggableTransportService {
  ...
  Uint8List _applyTlsCamouflage(Uint8List data) {
    ...
    builder.addByte(0x03); // Legacy Version Minor: TLS 1.3 handshake compatibility
  ...
  Uint8List _removeTlsCamouflage(Uint8List data) {
    if (data.length < 5) throw FormatException('Truncated TLS camouflage record');
    if (data[0] != 0x17 || data[1] != 0x03 || data[2] != 0x03) {
      throw FormatException('Invalid TLS camouflage header');
    }
    final length = (data[3] << 8) | data[4];
    if (data.length < 5 + length) {
      throw FormatException('TLS payload length mismatch');
    }
  ...
  Uint8List _removeShadowsocksAead(Uint8List data) {
    if (data.length < 16) throw FormatException('Truncated Shadowsocks payload');
  ...
  Uint8List _applyObfs4Entropy(Uint8List data) {
    ...
    final padding = Uint8List.fromList(List.generate(padLen, (_) => rnd.nextInt(256)));
    final builder = BytesBuilder();
  ...
  Uint8List _removeObfs4Entropy(Uint8List data) {
    if (data.isEmpty) throw FormatException('Empty obfs4 payload');
    final padLen = data[0];
    if (data.length < 1 + padLen) throw FormatException('Truncated obfs4 frame');
